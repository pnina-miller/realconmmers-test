...
import {GridItemCard, ListItemCard} from './components/itemCards'
...
function App() {
  ...
  return (
    <div className="App">
      <Router>
        <Switch>
          <Route exact path="/" render={() => <Viewdata data={data} getServerData={getServerData} />} />
...
function Viewdata({ data, getServerData }) {
    ...
    <Container style={{marginTop:'50px'}}>
      ...
      <Row  style={{marginTop:'50px'}}>
        {/* <Col xs={5}></Col> */}
          ...
          <input  style={{marginLeft:'20px', width:'auto'}} onChange={e => setCardsView(data.results.filter(item => item.Title.toLowerCase().includes(e.target.value.toLowerCase())))} />
          <img
      ...
      <Row>
        ...
        <Col xs={1}>
          <span title='grid' style={{ marginLeft: '10px' }} onClick={() => setGridView(true)}>
          ...
          <span title='list' style={{ marginLeft: '10px' }} onClick={() => setGridView(false)}>
